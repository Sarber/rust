...
pub mod write {
    ...
    pub fn run_passes(sess: Session,
        ...
        unsafe {
            ...
            let thresh = match sess.opts.optimize {
                session::No | session::Less => 225, // copied from clang
                session::Default | session::Aggressive => 275,
            };
            llvm::LLVMPassManagerBuilderSetOptLevel(builder, OptLevel as c_uint);
            llvm::LLVMPassManagerBuilderUseInlinerWithThreshold(builder, thresh);
            ...
            if sess.opts.save_temps {
                do output.with_filetype("opt.bc").with_c_str |buf| {
                    llvm::LLVMWriteBitcodeToFile(llmod, buf);
                }
            }
